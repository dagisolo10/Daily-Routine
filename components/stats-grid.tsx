...
export default function StatsGrid() {
    ...
    const statsDisplay = [
        { title: "Today's Tasks", value: `${stats.completedToday} / ${stats.totalTasks}`, subtitle: stats.completedToday === stats.totalTasks ? "Rituals completed! ✨" : `${stats.totalTasks - stats.completedToday} left to conquer`, color: "bg-blue-500/20 text-blue-600 shadow-blue-500/10" },
        { title: "Current Streak", value: `${stats.currentStreak} 🔥`, subtitle: stats.currentStreak > 0 ? "Don't break the chain! 🔗" : "Start your journey today", color: "bg-orange-500/20 text-orange-600 shadow-orange-500/10" },
    ...
    return (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {statsDisplay.map((stat) => (
                <Card key={stat.title} className={cn("justify-between gap-1 rounded-2xl p-4 shadow-xl backdrop-blur-3xl", stat.color)}>
                            ...
                            {Array.from({ length: stats.totalTasks }).map((_, key) => {
                                ...
                                return (
                                    <div key={key} className="h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800">
                                        <motion.div className={cn("h-full rounded-full bg-linear-to-r", isFilled ? "from-blue-500 to-purple-500" : "bg-transparent")} initial={{ width: "0%" }} animate={{ width: isFilled ? "100%" : "0%" }} transition={{ duration: 0.6, delay: key * 0.3, ease: "easeOut" }} />
                                    </div>
                                );
                            })}
                        </div>
