...
export default function Navbar() {
    ...
    const navigation = [{ label: "Home", icon: Home, path: "/" }, { label: "Milestones", icon: Target, path: "/milestones" }, { type: "fab" }, { label: "Profile", icon: User2, path: "/profile" }, { label: "Settings", icon: Settings, path: "/settings" }];

    return (
        <div className="fixed bottom-6 left-1/2 z-100 w-[92%] max-w-md -translate-x-1/2">
            <div className="flex items-center justify-around rounded-[2.5rem] border border-white/20 p-0.5 px-3 shadow-2xl backdrop-blur-2xl dark:border-zinc-800 dark:bg-black/70">
                {navigation.map((nav) => {
                    if (nav.type === "fab") {
                        return (
                            <div key="fab-container" className="relative -top-3">
                                <TaskModal />
                            </div>
                        );
                    }
                    ...
                    return (
                        <Link href={nav.path!} key={nav.path} className="relative flex flex-col items-center justify-center px-3 py-2 transition-all active:scale-90">
                            {active && <motion.div layoutId="nav-glow" className="absolute inset-0 z-0 rounded-3xl bg-blue-500/10 dark:bg-blue-500/20" transition={{ type: "spring", bounce: 0.3, duration: 0.6 }} />}
