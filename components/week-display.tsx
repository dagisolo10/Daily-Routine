    ...
    return (
        <div className="flex items-center justify-between">
            {relativeDays.map((item, index) => {
                ...
                return (
                    <motion.div key={index} animate={{ y: [-10, 0, -10] }} transition={{ duration: 4, ease: "easeInOut", repeat: Infinity, delay: index * 0.1 }} className="flex flex-col items-center gap-3">
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={cn("relative flex flex-col items-center gap-3 rounded-[24px] p-3 transition-all duration-500", isToday ? "z-10 scale-110 bg-linear-to-b from-blue-500 to-indigo-800 text-white shadow-[0_10px_30px_-10px_rgba(59,130,246,0.5)]" : "bg-card text-muted-foreground border border-white/10 backdrop-blur-md")}>
